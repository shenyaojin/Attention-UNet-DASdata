...
MODEL_NAME = 'ATTENTION_UNET'  # Options: 'UNET', 'ATTENTION_UNET'
imgPath = 'scripts/project/data/img/'
lblPath = 'scripts/project/data/lbl/'
picPath = 'scripts/project/picture/'
modPath = 'scripts/project/model/'
...
def train_model(model, criterion, optimizer, scheduler, epochs):
    ...
    for epoch in range(epochs):
        ...
        if train_loss[-1] < MinTrainLoss:
            file_path = modPath + f'/{model_name_str}_model_min_train.pth'
        ...
        if valid_loss[-1] < MinValidLoss:
            file_path = modPath + f'/{model_name_str}_model_min_valid.pth'
            state = {'net':model.state_dict(), 'optimizer':optimizer.state_dict(), 'epoch':epoch}
            torch.save(state, file_path)
        ...
        # save model
        if (epoch+1)%10 == 0:
            file_path = modPath + f'/{model_name_str}_model%03d.pth'%(epoch+1)
            state = {'net':model.state_dict(), 'optimizer':optimizer.state_dict(), 'epoch':epoch}
            torch.save(state, file_path)
    
    # plot
    picFile1 = picPath + f'{model_name_str}_loss.png'
    picFile2 = picPath + f'{model_name_str}_learning_rate.png'
    plot(title=f'{MODEL_NAME} Loss During Training', curve1=train_loss, curve2=valid_loss, xlabel='Epoch', ylabel='Loss', legend=['train', 'valid'], picFile=picFile1)
    plot(title=f'{MODEL_NAME} Learning Rate During Training', curve1=lr_list, xlabel='Epoch', ylabel='Learning Rate', picFile=picFile2)
